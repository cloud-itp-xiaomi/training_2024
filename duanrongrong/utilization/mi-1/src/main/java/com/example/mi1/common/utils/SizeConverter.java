 ...
 * https://blog.csdn.net/xmtblog/article/details/27194021?spm=a2c6h.12873639.article-detail.4.47be1368p6vA3Y
 */
public enum SizeConverter {
     ...
     * @return 字符串
     */
    public static double formatFileSize(MultipartFile file, SizeUnit unit) {
        double fileSize = 0;
        long len = file.getSize();
        if (unit.equals(SizeUnit.B)) {
            fileSize = (double) len;
        } else if (unit.equals(SizeUnit.K)) {
            fileSize = (double) len / 1024;
        } else if (unit.equals(SizeUnit.M)) {
            fileSize = (double) len / 1048576;
        } else if (unit.equals(SizeUnit.G)) {
