...
import com.example.mi1.common.config.StorageClient;
...
public class DataController {
    ...
    @Autowired
    DataController(CPUUtilizationService cpuUtilizationService, MachineService machineService,StorageClient storageClient) {
    ...
    public CommonResult<?> postUtilizationData(@RequestBody UploadParam uploadParam) {
    ...
    public CommonResult<List<QueryVO>> getUtilizationData(@RequestParam(value = "endpoint") String endpoint,
    ...
    public CommonResult<LogQueryVO> getLogData(@RequestParam(value = "hostname") String hostname,
                                               ...
                                               @RequestParam(value = "current_page") Integer currentPage) {
        LogQueryVO res = storageClient.getCurrentStorage().load(hostname, fileName, currentPage, 10);
        return CommonResult.success(res, "ok");
    }

    @PostMapping(value = "/log/upload")
    public CommonResult<?> postLogData(@RequestBody Log log) {
        cpuUtilizationService.checkFile(log.getFile());
        storageClient.getCurrentStorage().save(log);
        return CommonResult.success(null, "ok");
    }

    @GetMapping(value = "/machine/start")
    public CommonResult<?> startMachine() {
        machineService.start();
        return CommonResult.success(null, "ok");
    }

    @GetMapping(value = "/machine/stop")
    public CommonResult<?> stopMachine() {
        machineService.stop();
        return CommonResult.success(null, "ok");
    }

    @GetMapping(value = "/log/storage/change")
    public CommonResult<?> changeLogStorage(@RequestParam(value = "type") String storageType) {
        storageClient.setType(storageType);
        return CommonResult.success(null, "ok");
    }
}
