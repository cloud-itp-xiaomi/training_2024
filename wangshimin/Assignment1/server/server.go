...
var (
    ...
    rdb = redis.NewClient(&redis.Options{ // 初始化一个新的Redis客户端
        Addr: "127.0.0.1:6379", // Redis服务器地址
    })
    db, dbErr = sql.Open("mysql", "root:123456@tcp(localhost:3306)/MyDBserver") // 连接到MySQL数据库
)

func init() {
    if dbErr != nil {
        log.Fatalf("Error occurred during MySQL connection. Error: %s", dbErr.Error())
    }

    if err := rdb.Ping(ctx).Err(); err != nil {
        log.Fatalf("Error occurred during Redis connection. Error: %s", err.Error())
    }
}
